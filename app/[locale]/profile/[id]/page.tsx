...
// @ts-ignore - importing from route segment directory
import EditProfileForm from "@/app/profile/[id]/EditProfileForm";
...
export default async function ProfilePage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const [{ count: jobsDone }, { count: tasksPosted }, { data: reviews }] = await Promise.all([
    supabase.from("bids").select("*", { count: "exact", head: true }).eq("runner_id", id).eq("status", "accepted"),
    supabase.from("tasks").select("*", { count: "exact", head: true }).eq("poster_id", id),
    supabase.from("reviews").select("*, reviewer:profiles!reviewer_id(full_name, city, created_at)").eq("reviewee_id", id).order("created_at", { ascending: false }).limit(20),
  ]);
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#FAFAF8" }}>
      <nav style={{ backgroundColor: "white", borderBottom: "1px solid #E7E5E4", padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          <a href={`/${locale}/dashboard`} style={{ fontSize: 14, color: "#78716C", textDecoration: "none" }}>Dashboard</a>
          {user && (
            <a href={`/${locale}/profile/${user.id}`} style={{ fontSize: 14, color: "#F97316", fontWeight: 600, textDecoration: "none" }}>My profile</a>
          )}
        </div>
      </nav>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 28, alignItems: "start" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ backgroundColor: "white", borderRadius: 20, padding: 28, border: "1px solid #E7E5E4", textAlign: "center" }}>
              <div style={{ width: 96, height: 96, borderRadius: "50%", backgroundColor: bgColor, color: textColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, fontWeight: 800, margin: "0 auto 16px" }}>
                {initials}
              </div>

              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 4 }}>
                <h1 style={{ fontSize: 22, fontWeight: 800, color: "#1C1917", margin: 0 }}>{name}</h1>
                {profile.verification_status === "verified" && (
                  <span title="Identity verified" style={{ display: "inline-flex", alignItems: "center", gap: 4, backgroundColor: "#EFF6FF", color: "#1D4ED8", fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 999, flexShrink: 0 }}>
                    ✓ Verified
                  </span>
                )}
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 16 }}>
                <span style={{ fontSize: 12, fontWeight: 700, padding: "4px 12px", borderRadius: 999, backgroundColor: profile.role === "runner" ? "#FFF7ED" : "#EFF6FF", color: profile.role === "runner" ? "#EA580C" : "#1D4ED8" }}>
              ...
              <div style={{ borderTop: "1px solid #F5F4F2", paddingTop: 14, marginTop: 4 }}>
                <p style={{ fontSize: 12, color: "#A8A29E" }}>Member since {memberSince(profile.created_at)}</p>
              </div>

              {isOwn && (
                <div style={{ marginTop: 14 }}>
                  <span style={{ fontSize: 12, color: "#16A34A", fontWeight: 600, backgroundColor: "#F0FDF4", padding: "4px 12px", borderRadius: 999 }}>✓ This is you</span>
                </div>
              )}
            </div>

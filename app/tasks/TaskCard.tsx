...
export default function TaskCard({ task }: { task: Task }) {
  ...
  return (
    <a href={`/tasks/${task.id}`} style={{ textDecoration: "none" }}>
      <div
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              ...
              {isOverdue && <span style={{ fontSize: 12, color: "#DC2626", backgroundColor: "#FEF2F2", padding: "3px 10px", borderRadius: 999, fontWeight: 600 }}>Overdue</span>}
            </div>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            <div style={{ fontSize: 13, color: bidCount > 0 ? "#16A34A" : "#78716C", fontWeight: bidCount > 0 ? 600 : 400 }}>
              {bidCount > 0 ? `${bidCount} offer${bidCount > 1 ? "s" : ""}` : "No offers yet"}
            </div>
          </div>
        </div>
      </div>

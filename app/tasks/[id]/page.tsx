        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ backgroundColor: "white", borderRadius: 20, padding: 32, border: "1px solid #E7E5E4" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <span style={{ backgroundColor: st.bg, color: st.color, fontSize: 12, fontWeight: 700, padding: "4px 12px", borderRadius: 999 }}>
                ● {st.label}
              </span>
              <span style={{ fontSize: 12, color: "#A8A29E" }}>{timeAgo(task.created_at)}</span>
            </div>

            <h1 style={{ fontSize: 24, fontWeight: 800, color: "#1C1917", letterSpacing: "-0.5px", marginBottom: 20, lineHeight: 1.3 }}>
              {task.title}
            </h1>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
              ...
              {deadline && (
                <InfoBlock
                  icon="⏰"
                  label="Deadline"
                  value={deadline.toLocaleDateString("en-CA", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                  warn={deadline < new Date() && task.status === "open"}
                />
              )}
            </div>
          ...
          {isPoster && bids && bids.length > 0 && (
            <div style={{ backgroundColor: "white", borderRadius: 20, padding: 24, border: "1px solid #E7E5E4" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#1C1917", marginBottom: 16, textTransform: "uppercase", letterSpacing: "0.5px" }}>
                Offers ({bids.length})
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {bids.map(bid => (
                  <div key={bid.id} style={{
                    padding: 16,
                    borderRadius: 12,
                    border: `2px solid ${bid.status === "accepted" ? "#22C55E" : "#E7E5E4"}`,
                    backgroundColor: bid.status === "accepted" ? "#F0FDF4" : "white",
                  }}>
...
function InfoBlock({ icon, label, value, highlight, warn }: { icon: string; label: string; value: string; highlight?: boolean; warn?: boolean }) {
  return (
    <div style={{ backgroundColor: "#FAFAF8", borderRadius: 10, padding: "12px 16px" }}>
      ...
      <p style={{ fontSize: 15, fontWeight: 700, color: highlight ? "#F97316" : warn ? "#DC2626" : "#1C1917" }}>
        {icon} {value}
      </p>
    </div>
  );

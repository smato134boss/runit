...
const CITIES = ["All cities", "Toronto", "Hamilton", "Vancouver", "Ottawa", "Calgary", "Mississauga", "Brampton", "Edmonton", "Winnipeg", "Montréal", "Québec", "Laval"];
...
export default function BrowsePage() {
  ...
  const t = {
    browse: isFr ? "Parcourir les tâches" : "Browse tasks",
    subtitle: isFr ? "Trouvez des tâches près de vous et envoyez votre meilleure offre." : "Find tasks near you and send your best offer.",
    loading: isFr ? "Chargement des tâches..." : "Loading tasks...",
    noTasks: isFr ? "Aucune tâche trouvée" : "No tasks found",
    noTasksHint: isFr ? "Essayez de changer la ville ou la catégorie." : "Try changing city or category filters.",
    offerSent: isFr ? "✓ Offre envoyée" : "✓ Offer sent",
    placeOffer: isFr ? "Placer une offre →" : "Place offer →",
    viewOffer: isFr ? "Voir l'offre" : "View offer",
    dashboard: isFr ? "Tableau de bord" : "Dashboard",
    myJobs: isFr ? "Mes emplois" : "My jobs",
    allCities: isFr ? "Toutes les villes" : "All cities",
    allCategories: isFr ? "Tous" : "All",
  };
  ...
  const displayCities = [t.allCities, ...CITIES.slice(1)];
  const displayCategories = [t.allCategories, ...CATEGORIES.slice(1)];
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#FAFAF8" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: "#1C1917", letterSpacing: "-1px", marginBottom: 4 }}>{t.browse}</h1>
          <p style={{ fontSize: 15, color: "#78716C" }}>{t.subtitle}</p>
        </div>

        <div style={{ display: "flex", gap: 12, marginBottom: 28, flexWrap: "wrap" }}>
          <select value={cityFilter === "All cities" ? t.allCities : cityFilter} onChange={e => setCityFilter(e.target.value === t.allCities ? "All cities" : e.target.value)}
            ...
            {displayCities.map(c => <option key={c}>{c}</option>)}
          </select>

          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {displayCategories.map((cat, i) => {
              const rawCat = i === 0 ? "All" : CATEGORIES[i];
              const isActive = catFilter === rawCat;
              return (
                <button key={cat} onClick={() => setCatFilter(rawCat)}
                  style={{ padding: "8px 14px", borderRadius: 999, border: `2px solid ${isActive ? "#F97316" : "#E7E5E4"}`, backgroundColor: isActive ? "#FFF7ED" : "white", color: isActive ? "#EA580C" : "#78716C", fontSize: 13, fontWeight: isActive ? 700 : 500, cursor: "pointer", transition: "all 0.15s" }}>
                  {cat}
                </button>
              );
            })}
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "80px 0", color: "#78716C" }}>{t.loading}</div>
        ) : !tasks.length ? (
          <div style={{ backgroundColor: "white", borderRadius: 20, padding: 60, border: "1px solid #E7E5E4", textAlign: "center" }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>🔍</div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1C1917", marginBottom: 8 }}>{t.noTasks}</h2>
            <p style={{ fontSize: 14, color: "#78716C" }}>{t.noTasksHint}</p>
          </div>
        ) : (
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        ...
                        {alreadyBid && <span style={{ fontSize: 12, color: "#16A34A", backgroundColor: "#F0FDF4", padding: "3px 10px", borderRadius: 999, fontWeight: 600 }}>{t.offerSent}</span>}
                      </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", justifyContent: "space-between", flexShrink: 0 }}>
                      <div style={{ fontSize: 22, fontWeight: 800, color: "#F97316" }}>${task.budget.toFixed(0)}</div>
                      <a href={`/${locale}/tasks/${task.id}`}
                        style={{ backgroundColor: alreadyBid ? "#F5F4F2" : "#F97316", color: alreadyBid ? "#78716C" : "white", padding: "10px 18px", borderRadius: 999, fontSize: 14, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" as const, transition: "all 0.15s" }}>
                        {alreadyBid ? t.viewOffer : t.placeOffer}
                      </a>
                    </div>
                  </div>
                </div>

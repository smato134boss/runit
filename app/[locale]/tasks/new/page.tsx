...
export default function NewTaskPage() {
  ...
  const t = {
    ...
    heading: isFr ? "Poster une tâche" : "Post a task",
    subtitle: isFr ? "Décrivez ce dont vous avez besoin — les coursiers de votre ville vous enverront leur meilleure offre." : "Describe what you need — runners in your city will send you their best offer.",
    whatNeed: isFr ? "De quoi avez-vous besoin ? *" : "What do you need done? *",
    placeholder: isFr ? "ex. « Aller chercher des fleurs à Montréal pour ma maman »" : 'e.g. "Pick up flowers in Toronto for my mom"',
    suggestions: isFr
      ? ["Aller chercher un colis", "Envoyer un cadeau", "Course d'épicerie", "Commission en ville"]
      : ["Pick up a package", "Send a gift", "Grocery run", "Errand in the city"],
    category: isFr ? "Catégorie *" : "Category *",
    location: isFr ? "Emplacement *" : "Location *",
    fromCity: isFr ? "Ville de départ" : "From city",
    toCity: isFr ? "Ville d'arrivée" : "To city",
    ifDelivery: isFr ? "(si livraison)" : "(if delivery)",
    selectCity: isFr ? "Sélectionner une ville" : "Select city",
    sameCity: isFr ? "Même ville" : "Same city",
    budgetDeadline: isFr ? "Budget et délai" : "Budget & Deadline",
    budget: isFr ? "Votre budget (CAD) *" : "Your budget (CAD) *",
    deadline: isFr ? "Date limite" : "Deadline",
    optional: isFr ? "(optionnel)" : "(optional)",
    details: isFr ? "Détails" : "Details",
    detailsPlaceholder: isFr ? "Ajoutez des détails supplémentaires pour aider les coursiers..." : "Add any extra details that will help runners understand exactly what you need...",
    cancel: isFr ? "Annuler" : "Cancel",
    submit: isFr ? "Poster la tâche — c'est gratuit →" : "Post task — it's free →",
    posting: isFr ? "Publication en cours..." : "Posting task...",
    free: isFr ? "La publication est gratuite. Les coursiers paient un petit frais pour vous envoyer leur offre." : "Posting is free. Runners pay a small fee to send you their offer.",
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#FAFAF8" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "48px 24px" }}>
        <div style={{ marginBottom: 36 }}>
          <h1 style={{ fontSize: 32, fontWeight: 800, color: "#1C1917", letterSpacing: "-1px", marginBottom: 8 }}>{t.heading}</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div style={{ backgroundColor: "white", borderRadius: 16, padding: 24, border: "1px solid #E7E5E4" }}>
            <label style={{ fontSize: 13, fontWeight: 700, color: "#1C1917", display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.5px" }}>{t.whatNeed}</label>
            <input type="text" value={form.title} onChange={e => update("title", e.target.value)} placeholder={t.placeholder} required style={inputStyle}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              <div>
                <label style={{ fontSize: 13, color: "#78716C", fontWeight: 500, display: "block", marginBottom: 6 }}>
                  {t.toCity} <span style={{ color: "#A8A29E" }}>{t.ifDelivery}</span>
                </label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input type="number" value={form.budget} onChange={e => update("budget", e.target.value)} placeholder="0.00" required min="5" step="0.01"
          ...
          <div style={{ backgroundColor: "white", borderRadius: 16, padding: 24, border: "1px solid #E7E5E4" }}>
            ...
            <textarea value={form.description} onChange={e => update("description", e.target.value)} placeholder={t.detailsPlaceholder} rows={4}
              style={{ ...inputStyle, resize: "vertical" as const, lineHeight: 1.6 }}
              onFocus={e => e.target.style.borderColor = "#F97316"} onBlur={e => e.target.style.borderColor = "#E7E5E4"} />
          </div>

          <div style={{ display: "flex", gap: 12 }}>
            <a href={`/${locale}/dashboard`} style={{ flex: 1, padding: "14px", borderRadius: 12, backgroundColor: "white", color: "#1C1917", fontSize: 15, fontWeight: 600, border: "2px solid #E7E5E4", textDecoration: "none", textAlign: "center" }}>
              {t.cancel}
            </a>
          ...
          <p style={{ textAlign: "center", fontSize: 13, color: "#78716C" }}>{t.free}</p>
        </form>
      </div>
    </div>
  );

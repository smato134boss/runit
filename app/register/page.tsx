        ...
        <div style={{ width: "100%", maxWidth: 520 }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 36 }}>
            <h1 style={{ fontSize: 32, fontWeight: 800, color: "#1C1917", letterSpacing: "-1px", marginBottom: 8 }}>
              {step === 1 ? "Join runit" : `Welcome, ${form.name.split(" ")[0] || "there"} 👋`}
            </h1>
            <p style={{ fontSize: 15, color: "#78716C" }}>
              {step === 1 ? "First, tell us how you want to use runit" : "Just a few more details to get started"}
            </p>
          </div>
          ...
          <div style={{ backgroundColor: "white", borderRadius: 24, padding: 40, border: "1px solid #E7E5E4", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
            ...
            {step === 1 && (
              <div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        padding: "20px 16px",
                        borderRadius: 14,
                        border: `2px solid ${role === opt.value ? "#F97316" : "#E7E5E4"}`,
                        backgroundColor: role === opt.value ? "#FFF7ED" : "white",
                        cursor: "pointer",
                        textAlign: "center",
                        transition: "all 0.2s",
                      }}
                ...
                <button
                  onClick={() => { if (form.name && form.email && role) setStep(2); }}
                  disabled={!form.name || !form.email || !role}
                  style={{ width: "100%", padding: "14px", borderRadius: 12, backgroundColor: (!form.name || !form.email || !role) ? "#FED7AA" : "#F97316", color: "white", fontSize: 16, fontWeight: 700, border: "none", cursor: (!form.name || !form.email || !role) ? "not-allowed" : "pointer", transition: "all 0.2s", marginTop: 20 }}
                  onMouseEnter={e => { if (form.name && form.email && role) e.currentTarget.style.backgroundColor = "#EA580C"; }}
                  onMouseLeave={e => { if (form.name && form.email && role) e.currentTarget.style.backgroundColor = "#F97316"; }}>
                  Continue →
                </button>
              </div>
            )}
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "#1C1917", display: "block", marginBottom: 6 }}>
                    Phone number <span style={{ color: "#78716C", fontWeight: 400 }}>(optional)</span>
                  </label>
                  <input
                    type="tel"
                    value={form.phone}
                ...
                <div style={{ backgroundColor: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 600, color: "#EA580C" }}>
                      Joining as: {role === "poster" ? "Task Poster" : "Runner"}
                    </p>
                    <p style={{ fontSize: 12, color: "#78716C" }}>You can switch anytime in your profile</p>
                  </div>
                </div>
